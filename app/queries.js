...
function getDriversPerSeason(request,response){
	var query = "SELECT year, seasonurl as url, json_agg(json_build_object('driver',drivers.*,'points',total_points)) as top_drivers "+
		"FROM (SELECT year, seasons.url AS seasonurl, driver_id, SUM(results.points) as total_points, RANK() OVER(PARTITION BY year ORDER BY SUM(results.points) DESC) rank "+
		"FROM seasons "+
...
function getDriverRacesByIdOrName(request,response){
	...
	var query = "SELECT drivers.*, json_agg(json_build_object('race_id',race_id,'average_lap_time',avg_lap_time,'fastest_lap_time',min_lap_time,'slowest_lap_time',max_lap_time,'pit_stops',pit_stops_count,"+
						"'fastest_pit_stop',min_pit_stop,'slowest_pit_stop',max_pit_stop,'circut_name',circut_name,'points',results_points,'position',results_position)) as races "+
				"FROM drivers "+
				"INNER JOIN "+
				"(SELECT race_id, driver_id, MAX(pit_stops.stop) as pit_stops_count, MAX(pit_stops.time) max_pit_stop, MIN(pit_stops.time) min_pit_stop,"+
					   "AVG(lap_times.time) avg_lap_time, MAX(lap_times.time) max_lap_time, MIN(lap_times.time) min_lap_time, circuits.name as circut_name, results.points as results_points,results.position as results_position "+
				"FROM results "+
				"LEFT JOIN races USING(race_id) "+
